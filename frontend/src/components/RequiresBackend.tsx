...
export function BackendRequiredModal({ show, onClose, feature }: { show: boolean; onClose: () => void; feature: string }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-2xl animate-slide-up" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold text-dark-900">
            <span className="text-amber-500 mr-1">⚡</span> Local backend required
          </h3>
          <button onClick={onClose} className="text-dark-400 hover:text-dark-600 text-lg">&times;</button>
        </div>
        ...
        <div className="rounded-lg bg-dark-50 p-4 mb-4">
          <p className="text-xs font-semibold text-dark-700 mb-2">Quick setup — 2 minutes</p>
