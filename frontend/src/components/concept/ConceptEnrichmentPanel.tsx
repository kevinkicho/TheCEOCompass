...
import {
  explainConcept, generateWhyItMatters, buildWhyItMattersPrompt, generateHowToApply, buildHowToApplyPrompt,
  generateCommonPitfalls, buildCommonPitfallsPrompt, generateConnectedConcepts, buildConnectedConceptsPrompt,
  generateCaseStudy, buildCaseStudyPrompt, generateTestYourself, buildTestYourselfPrompt,
  generateRealWorldExamples, buildRealWorldExamplesPrompt, buildExplainPrompt, loadCategoryEntries,
} from "@/lib/ollama"
...
const CATS = [
  "explain_further", "why_it_matters_for_ceos", "how_to_apply", "common_pitfalls",
...
export function ConceptEnrichmentPanel({ framework, concept, frameworkSlug, conceptSlug }: Props) {
  const { isAdmin } = useAuth()
  const slug = frameworkSlug
  ...
  const goToCatPage = (cat: string, idx: number) => {
    ...
    try {
      ...
      if (["why_it_matters_for_ceos", "how_to_apply", "common_pitfalls", "connected_concepts"].includes(cat)) {
        setAiEnrichment((prev: any) => ({ ...prev, ...parsed }))
      } else if (cat === "case_study") setAiCaseStudy(parsed)
      else if (cat === "test_yourself") setAiExercise(parsed)
      else if (cat === "real_world_examples") setAiExample(parsed)
    } catch { /* ignore */ }
  }
  ...
  const handleNewEntry = useCallback((cat: string, id: string, data: any) => {
    ...
    setCatEntries((d) => {
      const prev = d[cat] || []
      if (prev.some((e: any) => e.id === id)) return d
      return { ...d, [cat]: [entry, ...prev].sort((a, b) => (b.created_at || 0) - (a.created_at || 0)) }
    })
  }, [])
    ...
    ;(async () => {
      ...
      for (let i = 0; i < CATS.length; i++) {
        ...
        try {
          ...
          if (cat === "explain_further") setAiPromptText(latest.prompt || "")
  ...
  useEffect(() => {
    ...
    const unsubs = CATS.map((cat) =>
      onChildAdded(ref(db!, `framework/${slug}/${conceptSlug}/${cat}`), (snap) => {
  ...
  const promptWhy = buildWhyItMattersPrompt(concept.name, concept.definition, framework.title, concept.tags)
  const promptHow = buildHowToApplyPrompt(concept.name, concept.definition, framework.title)
  const promptPitfalls = buildCommonPitfallsPrompt(concept.name, concept.definition, framework.title)
  const promptConnected = buildConnectedConceptsPrompt(concept.name, concept.definition, framework.title)
  const promptCaseStudy = buildCaseStudyPrompt(concept.name, concept.definition, framework.title)
  const promptExercise = buildTestYourselfPrompt(concept.name, concept.definition, framework.title)
  const promptExample = buildRealWorldExamplesPrompt(concept.name, concept.definition, framework.title)
  const promptExplain = buildExplainPrompt(concept.name, concept.definition, framework.title)

  const mkHandler = (
    generator: Function, setter: Function, setCached: Function, setLoading: Function, setError?: Function, ...args: any[]
  ) => async () => {
    setLoading(true)
    setter(null)
  ...
  const handleExplain = mkHandler(
    explainConcept, () => setEditingPrompt(false), () => {}, (v: boolean) => setAiLoading(v),
    (e: string) => setAiError(e), concept.name, concept.definition, slug, framework.title, true,
  )
  const handleWhyItMatters = mkHandler(
    generateWhyItMatters, (p: any) => setAiEnrichment((prev: any) => ({ ...prev, ...p })), () => {}, () => {},
    undefined, concept.name, concept.definition, slug, framework.title, true, concept.tags,
  )
  const handleHowToApply = mkHandler(
    generateHowToApply, (p: any) => setAiEnrichment((prev: any) => ({ ...prev, ...p })), () => {}, () => {},
    undefined, concept.name, concept.definition, slug, framework.title, true,
  )
  const handleCommonPitfalls = mkHandler(
    generateCommonPitfalls, (p: any) => setAiEnrichment((prev: any) => ({ ...prev, ...p })), () => {}, () => {},
    undefined, concept.name, concept.definition, slug, framework.title, true,
  )
  const handleConnectedConcepts = mkHandler(
    generateConnectedConcepts, (p: any) => setAiEnrichment((prev: any) => ({ ...prev, ...p })), () => {}, () => {},
    undefined, concept.name, concept.definition, slug, framework.title, true,
  )
  const handleCaseStudy = mkHandler(
    generateCaseStudy, setAiCaseStudy, () => {}, (v: boolean) => setAiCaseStudyLoading(v),
    undefined, concept.name, concept.definition, slug, framework.title, true,
  )
  const handleExercise = mkHandler(
    generateTestYourself, setAiExercise, () => {}, (v: boolean) => setAiExerciseLoading(v),
    undefined, concept.name, concept.definition, slug, framework.title, true,
  )
  const handleExample = mkHandler(
    generateRealWorldExamples, setAiExample, () => {}, (v: boolean) => setAiExampleLoading(v),
    undefined, concept.name, concept.definition, slug, framework.title, true,
  )
