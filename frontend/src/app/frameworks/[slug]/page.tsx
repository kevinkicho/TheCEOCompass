  ...
  return (
    ...
    <div className="mx-auto max-w-4xl px-4 py-16">
      ...
      <button
        ...
        className="mb-6 inline-flex items-center gap-1 text-sm text-dark-500 hover:text-primary-600 transition"
      >
      ...
      <div className="mb-10">
        ...
        <h1 className="mb-3 text-4xl font-bold text-dark-900">{framework.title}</h1>
        <p className="text-lg text-dark-500">{framework.description}</p>
      </div>

      {/* Key Concepts — responsive grid with modal */}
      <div className="mb-8">
        <h2 className="mb-4 text-xl font-semibold text-dark-900">Key Concepts</h2>
            ...
            return (
              <button
                ...
                className={`rounded-lg border px-3.5 py-3 text-center w-full transition hover:border-primary-300 hover:shadow-sm ${
                  concept
                    ? "border-dark-200 cursor-pointer hover:bg-primary-50"
                    : "border-dark-100 bg-dark-50 cursor-default opacity-60"
                }`}
              >
                <span className="text-[13px] sm:text-sm font-medium text-dark-700 leading-snug break-words">
                  {name}
                </span>
              </button>
            )
      ...
      {/* Use Cases */}
      <div className="mb-8">
        <h2 className="mb-4 text-xl font-semibold text-dark-900">When to Use</h2>
        <div className="flex flex-wrap gap-2">
          {framework.use_cases?.map((useCase) => (
            <span key={useCase} className="rounded-full bg-dark-100 px-3 py-1 text-sm text-dark-700">
              {useCase}
            </span>
          ))}
        </div>
      ...
      {scenarios.length > 0 && (
        <div className="mb-8">
          <h2 className="mb-4 text-xl font-semibold text-dark-900">Practice Scenarios</h2>
    ...
    {modalConcept && (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setModalConcept(null)}>
        <div
          className="w-full max-w-lg max-h-[85vh] overflow-y-auto rounded-xl bg-white p-6 shadow-2xl animate-slide-up"
        ...
        >
          ...
          {modalConcept.example && (
            <div className="mb-4">
              <p className="text-xs font-semibold text-dark-400 uppercase tracking-wide mb-3">Real-World Examples</p>
              <ul className="space-y-3">
                {modalConcept.example.split(" | ").map((ex, i) => (
                  <li key={i} className="flex items-start gap-3 text-sm text-dark-700 bg-dark-50 rounded-lg p-3">
